...
//!
//! An upstream mugle stratum server
//!
...
// ----------------------------------------
// Server Object - our connection to a stratum server - a mugle node
...
impl Server {
    ...
    pub fn new(cfg: Config) -> Server {
        Server {
            id: "MuglePool".to_string(),
            config: cfg,
            stream: None,
            protocol: StratumProtocol::new(),
            error: false,
            job: JobTemplate::new(),
            status: WorkerStatus::new("MuglePool".to_string()),
            buffer: String::with_capacity(4096),
        }
    }

    /// Connect to an upstream Mugle Stratum Server
    ...
    pub fn connect(&mut self) -> Result<(), String> {
        ...
        let mugle_stratum_url = self.config.mugle_node.address.clone() + ":"
            + &self.config.mugle_node.stratum_port.to_string();
        warn!(
            "{} - Connecting to upstream stratum server at {}",
            self.id,
            mugle_stratum_url.to_string()
        );
        match TcpStream::connect(mugle_stratum_url.to_string()) {
            Ok(conn) => {
    ...
    /// Request status from the upstream Mugle Stratum server - this is *pool* status (not individual
    /// worker status)
    // Currently unused
...
//    }
        ...
        match self.stream {
            Some(ref mut stream) => {
                let login_params = LoginParams {
                    login: self.config.mugle_node.login.clone().to_string(),
                    pass: self.config.mugle_node.password.clone().to_string(),
                    agent: self.id.clone(),
                };
    ...
    /// Request a new job template from the upstream Mugle Stratum server
                        ...
                        match rpc_msg {
                            Some(message) => {
                                ...
                                if v["method"] == String::from("job") {
                                    ...
                                    // This is a RESPONSE from the upstream Mugle Stratum Server
                                    ...
                                    if worker_id == self.id {
                                        trace!("RESPOPNSE for MUGLEPOOL: {}", worker_id);
