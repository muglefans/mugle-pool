...
use failure::Error;
use mugle_util::from_hex;
use mugle_core::pow::Proof;
use mugle_core::core::BlockHeader;
use mugle_core::global::{ChainTypes, set_mining_mode};
use mugle_core::ser::{deserialize, ser_vec};
...
fn block_header(pre_pow: String,
    ...
    let mut pv = mugle_core::ser::ProtocolVersion(2);
...
fn pool_to_chaintype(pool: String) -> ChainTypes {
    ...
    match pool.as_str() {
        "muglepool" => { return ChainTypes::Mainnet; },
        "mwmuglepool" => { return ChainTypes::Mainnet; },
        "mwfloopool" => { return ChainTypes::Floonet; },
        "bitmugle" => { return ChainTypes::Mainnet; },
...
impl Pool {
    /// Create a new Mugle Stratum Pool
    pub fn new(config: Config) -> Pool {
        Pool {
            id: "Mugle Pool".to_string(),
            job: JobTemplate::new(),
            config: config.clone(),
            ...
            chain_type: pool_to_chaintype(config.mugle_pool.pool.clone()),
        }
    }
    ...
    debug!(
        ...
        e
                        ...
                        // B) Call into mugle_core::pow::verify_size()
                                let verify_result = mugle_core::pow::verify_size(&bh);
                                if ! verify_result.is_ok() {
                                        worker.status.rejected += 1;
                                        worker.add_shares(share.edge_bits, 0, 1, 0); // Accepted, Rejected, Stale
    debug!(
        "{} - Rejected due to failed mugle_core::pow::verify_size()",
        self.id,
    );
                                        worker.send_err("submit".to_string(), "Failed to validate solution".to_string(), -32502);
                                        continue; // Dont process this share anymore
                        ...
                        // This is a good share, send it to mugle server to be submitted
