...
# Get the latest mugle block
while True:
    latest = json.loads(requests.get("http://192.168.2.224:13423/mugle/block").content.decode('utf-8'))
    if latest != None:
        break
# print("Latest: {}".format(latest))
# Get the previous mugle block
prev_height = int(latest["height"]) - 1
previous = json.loads(requests.get("http://192.168.2.224:13423/mugle/block/{}".format(prev_height)).content.decode('utf-8'))
# print("Previous: {}".format(previous))
# Get the latest mugle stats
#stats = json.loads(requests.get("http://mugle-pool.us:13423/mugle/stat").content.decode('utf-8'))
stats = json.loads(requests.get("http://192.168.2.224:13423/mugle/stats/0,5".format(prev_height)).content.decode('utf-8'))
...
#pool_stats = json.loads(requests.get("http://mugle-pool.us:13423/pool/stat").content.decode('utf-8'))
...
#active_miners = json.loads(requests.get("http://mugle-pool.us:13423/worker/stats/{}/worker".format(pool_stats["height"])).content.decode('utf-8'))
...
#        print("share: {}".format(worker_shares[-1]))
    ...
    print("Balance: {} mugle".format(worker_stats["balance"]))
    print("Last Paid: {}".format(time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(worker_payment_info["last_success"]))))
    print("Total Paid: {}".format(worker_stats["total_mugle_paid"]))
    print("-------------------------------")
