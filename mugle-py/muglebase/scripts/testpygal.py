...
def get_weather_data(start='0', r='120'):
    url = 'http://mugle-pool.us:13423/mugle/stats/' + start +','+r+'/gps,height'
    result = urlopen(url)
...
#    bar_chart.x_labels = times
    print("Num mugle stats: {}".format(len(imp_temps)))
    bar_chart.add('Mugle GPS', imp_temps)
    ...
    url = 'http://mugle-pool.us:13423/pool/stats/' + start +','+r+'/gps,height'
    result = urlopen(url)
    js_string = result.read().decode('utf-8')
    parsed = json.loads(js_string)
    imp_temps = [float(i['gps']) for i in parsed]
    ...
    try:
        url = 'http://mugle-pool.us:13423/worker/stats/' + start +','+r+'/worker,gps,height'
        result = urlopen(url)
        js_string = result.read().decode('utf-8')
        parsed = json.loads(js_string)
