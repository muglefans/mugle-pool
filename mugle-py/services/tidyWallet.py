...
def cancel_expired_tx(database, mugle_owner_api_url, tx_timeout):
    global LOGGER
    global CONFIG
...
def confirm_tx(database, mugle_owner_api_url):
    global LOGGER
    global CONFIG
...
# Re-post a posted tx
def repost_tx(database, mugle_owner_api_url, tx_timeout_delta):
    global LOGGER
    global CONFIG
...
def main():
    ...
    database = lib.get_db()
    mugle_owner_api_url = mugle.get_owner_api_url()
    ...
    try:
        repost_tx(database, mugle_owner_api_url, tx_repost_delta)
    except Exception as e:
