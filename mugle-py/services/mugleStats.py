...
# Add a mugle network stats record ~per block
...
def main():
    ...
    last_height = 0
    latest_stat = Mugle_stats.get_latest()
    print("latest_stat = {}".format(latest_stat))

    if latest_stat == None:
        LOGGER.warn("Initializing Mugle_stats")
        muglestats.initialize(avg_over_range, LOGGER)
        latest_stat = Mugle_stats.get_latest()
        print("Finished initializing, latest_stat height = {}".format(latest_stat.height))
    last_height = latest_stat.height
    height = last_height + 1
    LOGGER.warn("mugleStats service starting at block height: {}".format(height))

    # Generate mugle stats records - one per mugle block
    while True:
        #latest_db_block = Blocks.get_latest()
        latest = Blocks.get_latest().height
        while latest >= height:
            try:
                new_stats = muglestats.calculate(height, avg_over_range)
...
#                if( (height % BATCHSZ == 0) or (height >= (latest-10)) ):
                ...
                LOGGER.warn("Added Mugle_stats for block: {} - gps:{} diff:{}".format(new_stats.height, new_stats.gps, new_stats.difficulty))
                height = height + 1
