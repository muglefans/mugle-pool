...
#
# Routines for getting Mugle Pool data
#
...
# Globals
NANOMUGLE = 1000000000 # 1 and 9 zeros
...
def init_pool_users(CONFIG, LOGGER, database):
    ...
    try:
        webui_user = os.environ["MUGLE_POOL_WEBUI_USER"]
        webui_pass = os.environ["MUGLE_POOL_WEBUI_PASSWORD"]
    except KeyError:
...
def get_block_reward_nanomugle():
    BLOCK_REWARD_NANOMUGLE = get_block_reward() * NANOMUGLE
    return BLOCK_REWARD_NANOMUGLE
...
def get_reward_by_block(height):
    ...
    if theblock is None:
        return 0;
    return get_block_reward_nanomugle() + theblock.fee
...
# Calculate expected mugle reward per day for 'gps' at 'height' using 'range' avg data
def estimated_daily_reward(gps=1, height=0, range=60):
    if height == 0:
        height=mugle.blocking_get_current_height()
    # Get Avg Network GPS
    network_gps_avgs = muglestats.avg_network_gps(height, range)
    # Get POW Ratios
    C29Ratio = mugle.secondary_pow_ratio(height)
    PrimaryRatio = 100 - C29Ratio
    # Calculate Expected Mugle Reward per Day
    estReward = {}
    for powsize, gpsavg in network_gps_avgs.items():
        if powsize == mugle.SECONDARY_SIZE:
            estReward[powsize] = float(gps) / network_gps_avgs[powsize] * C29Ratio/100.0 * mugle.DAY_HEIGHT * get_block_reward()
        else:
            estReward[powsize] = float(gps) / network_gps_avgs[powsize] * PrimaryRatio/100.0 * mugle.DAY_HEIGHT * get_block_reward()
    return estReward
...
def calculate_block_payout_map(height, window_size, pool_fee, logger, estimate=False):
    block_payout_map = {}
    # Get the muglepool admin user ID for pool fee
...
#        }
    ...
    for user_id, worker_shares_count in shares_count_map.items():
        ...
        if user_id in block_payout_map.keys():
            ...
            block_payout_map[user_id] = worker_payment
    logger.warn("Total Mugle Paid Out this block: {} + the_pools_fee: {} ".format(total_payments_this_block, the_pools_fee))
