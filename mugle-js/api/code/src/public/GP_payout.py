...
class Pool_Payout:
    ...
    def find_wallet(self):
        ##
        # Find Mugle Wallet Command
        mugle_wallet_cmd = None
        cwd = os.getcwd()
        ...
        for directory in path.split(":"):
            if os.path.isfile(directory + "/{}-wallet".format(self.walletprefix)):
                mugle_wallet_cmd = [directory + "/{}-wallet".format(self.walletprefix)]
            elif os.path.isfile(directory + "/{}-wallet.exe".format(self.walletprefix)):
                mugle_wallet_cmd = [directory + "/{}-wallet.exe".format(self.walletprefix)]
        if mugle_wallet_cmd is None:
        ...
        if self.walletflags is not None:
            mugle_wallet_cmd = mugle_wallet_cmd + [ self.walletflags ]
    
        # Wallet Sanity Check
        wallettest_cmd = mugle_wallet_cmd + [ "-p", self.wallet_pass, "info" ]
        message = None
        try:
            message = subprocess.check_output(wallettest_cmd, stderr=subprocess.STDOUT, shell=False)
            self.wallet_cmd = mugle_wallet_cmd
            return(message)
    ...
    def get_balance(self):
        ...
        if r.json() is None:
            ...
            balance_nanomugle = r.json()["amount"]
        self.balance = balance_nanomugle / 1000000000.0
        if self.balance < 0:
            self.balance = 0.0
    ...
    ##
    # Get Payout using local mugle wallet
    def run_mugle_wallet(self):
        ...
        # Find wallet Command
        self.print_progress("Locating your mugle wallet command");
...
#            xxx
        ...
        # Call mugle wallet to receive the slate and sign it
...
#        fh.write(self.signed_slate)
    ...
    def run(self):
        ...
        parser.add_argument("--wallet_pass", help="Your mugle wallet password")
