...
import { DatabaseMugleBlock, DatabaseMugleStat } from '../types/types'
...
networkRouter.get('/test', async (req, res, next) => {
  console.log('/mugle/test touched')
  res.json('success')
})
...
networkRouter.get('/block', async (req, res, next) => {
  try {
    ...
    const latestBlockResults: DatabaseMugleBlock[] = await knex.select(['*', knex.raw('UNIX_TIMESTAMP(timestamp) as timestamp')])
      .from('blocks')
...
networkRouter.get('/block/:height?/:fields?', sanitizeHeight, async (req, res, next) => {
  try {
    let blockResult: DatabaseMugleBlock
...
networkRouter.get('/blocks/:height,:range/:fields?', sanitizeHeight, limitRange, async (req, res, next) => {
  try {
    ...
    let blockRangeResults: DatabaseMugleBlock[]
    if (fields) {
...
networkRouter.get('/stats/:height,:range/:fields?', sanitizeHeight, limitRange, async (req, res, next) => {
  try {
    ...
    const networkStatsRangeResults: DatabaseMugleStat[] = await knex
      .select(['mugle_stats.difficulty', 'gps.gps', 'gps.edge_bits', 'blocks.height', knex.raw('UNIX_TIMESTAMP(mugle_stats.timestamp) as timestamp'), 'blocks.secondary_scaling'])
      .from('mugle_stats')
      .join('gps', 'mugle_stats.height', '=', 'mugle_stats_id')
      .join('blocks', 'mugle_stats.height', '=', 'blocks.height')
      .where('mugle_stats.height', '>', min)
      .andWhere('mugle_stats.height', '<=', finalHeight)
    // console.log('/mugle/stats/height,range/fields results are: ', networkStatsRangeResults)
...
networkRouter.get('/stats/:height/:fields?', sanitizeHeight, async (req, res, next) => {
  try {
    ...
    const networkStatResult: DatabaseMugleStat[] = await knex.select(['*', knex.raw('UNIX_TIMESTAMP(timestamp) as timestamp')])
      .from('mugle_stats')
      .join('gps', 'height', 'mugle_stats_id')
