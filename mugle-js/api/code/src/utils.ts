...
import { FindUserResult, BlockGpsStat, MergedBlockGpsStat, MergeBlocksInput, WorkerShareStat, PoolStat, DatabaseMugleStat } from './types/types'
...
export const calculateBlockReward = (height: number, workerStats: WorkerShareStat[], poolStats: PoolStat[]) => {
  ...
  const userReward =  mugleToNanoMugle(60) * portion
...
export const sanitizeHeight = async (req, res, next) => {
  ...
  if (req.params && req.params.height && (parseInt(req.params.height) < 1)) {
    ...
    index.cache.get(`router_/mugle/block`, async (err, item: string) => {
      ...
      if (item) {
        ...
        index.cache.set(`router_/mugle/block.height`, maxHeight, 'EX', 1)
...
export const nanoMugleToMugle = (nanoMugle: number): number => {
  return nanoMugle / 1000000000
}

export const mugleToNanoMugle = (mugle: number): number => {
  return mugle * 1000000000
}