...
import { fetchMuglePoolBlocksMined } from './muglePoolDataActions.js'
...
export const fetchNetworkData = (start: number = 0) => async (dispatch: Dispatch, getState: GetState) => {
  try {
    ...
    const url = `${API_URL_V2}mugle/stats/${latestBlockHeight},${blockDifference}/gps,height,difficulty,timestamp,secondary_scaling`
...
export const getLatestBlock = () => async (dispatch: Dispatch, getState: GetState) => {
  try {
    ...
    const latestBlockUrl = `${API_URL_V2}mugle/block`
...
export const fetchNetworkRecentBlocks = (endBlock: number | null = null, range: number = 20) => async (dispatch: Dispatch, getState: GetState) => {
  try {
    ...
    if (latestBlockHeight === 0) return
    const url = `${API_URL_V2}mugle/blocks/${latestBlockHeight},${range}`
    ...
    // const blocksOrphaned = muglePoolRecentBlocksData.filter((block) => block.state === 'orphan')
    // console.log('fetchMuglePoolRecentBlocks blocksOrphaned: ', blocksOrphaned)
    dispatch({
...
export const getMinedBlocksAlgos = () => async (dispatch: Dispatch, getState: GetState) => {
  try {
    ...
    const blockDifference = latestBlockHeight - previousMaxBlockHeight
    const minedBlockAlgosUrl = `${API_URL_V2}mugle/blocks/${latestBlockHeight},${blockDifference}/height,edge_bits`
    ...
    dispatch(fetchMuglePoolBlocksMined())
  } catch (e) {
    console.log('error: ', e)
  }
}
