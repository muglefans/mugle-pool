...
type LatestBlockMugleEarnedAction = { type: 'MINER_LATEST_BLOCK_MUGLE_EARNED', data: number } | { type: 'ACCOUNT', data: null }

const latestBlockMugleEarned = (state: number = 0, action: LatestBlockMugleEarnedAction) => {
  switch (action.type) {
    case 'MINER_LATEST_BLOCK_MUGLE_EARNED':
      return action.data
    case 'ACCOUNT':
      if (action.data === null) {
        return 0
...
type NextBlockMugleEarnedAction = { type: 'MINER_Next_BLOCK_MUGLE_EARNED', data: number } | { type: 'ACCOUNT', data: null }

const nextBlockMugleEarned = (state: number = 0, action: NextBlockMugleEarnedAction) => {
  switch (action.type) {
    case 'MINER_NEXT_BLOCK_MUGLE_EARNED':
      return action.data
    case 'ACCOUNT':
      if (action.data === null) {
        return 0
