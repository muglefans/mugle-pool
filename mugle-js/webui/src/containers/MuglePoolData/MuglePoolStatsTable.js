...
export class MuglePoolStatsTableComponent extends Component {
  UNSAFE_componentWillMount () {
    this.fetchMuglePoolTableStats()
  }
  ...
  fetchMuglePoolTableStats = () => {
    const { fetchNetworkData } = this.props
    fetchNetworkData()
  }

  render () {
    const { historicalMuglePoolData, historicalNetworkData } = this.props
    ...
    const poolBlockIndex = historicalMuglePoolData.length - 1
    const poolBlock = historicalMuglePoolData[poolBlockIndex]
    if (historicalMuglePoolData[poolBlockIndex]) {
