...
export class MuglePoolDataComponent extends Component {
  interval = null
  constructor (props) {
    super(props)
    this.state = {
  ...
  fetchMuglePoolData = () => {
    const { fetchMuglePoolData, fetchMuglePoolLastBlock } = this.props
    fetchMuglePoolData()
  ...
  render () {
    const { faderStyleId } = this.state
    const { muglePoolData, activeWorkers, lastBlockMined, poolBlocksMined, latestBlockHeight } = this.props
    ...
    let c29BlockCount = 0
    let c31BlockCount = 0
    if (muglePoolData.length > 0) {
      const lastBlock = muglePoolData[muglePoolData.length - 1]
    ...
    const totalPoolBlocksMined = muglePoolData[muglePoolData.length - 1] ? muglePoolData[muglePoolData.length - 1].total_blocks_found : 0
      ...
      <Row xs={12} md={12} lg={12} xl={12}>
        <Col xs={12} md={12} lg={12} xl={12}>
          <MiningGraphConnector
            miningData={muglePoolData}
            poolBlocksMined={poolBlocksMined}
            decimals={1}
          />
        </Col>
        <Col xs={12} md={12} lg={12} xl={12}>
          <h4 className='page-title' style={{ marginBottom: 36 }}>MUGLE-Pool Stats</h4>
          <Table size='sm'>
            <tbody>
              <tr>
