...
import { nanoMugleToMugle, getMinerBlockRewardData, calculateDailyEarningFromGps } from '../../utils/utils.js'
...
export type MinerDataComponentStateProps = {
    ...
    muglePoolData: Array<Object>,
    muglePoolRecentBlocks: Array<Object>
}
...
export type MinerDataComponentDispatchProps = {
    ...
    fetchMuglePoolData: () => void,
    fetchMuglePoolRecentBlocks: () => void
}
...
export class RigDataComponent extends Component<MinerDataComponentProps, MinerDataComponentState> {
  ...
  render () {
    ...
    muglePoolData.forEach(block => {
      if (block.height > maxPoolBlockHeight) maxPoolBlockHeight = block.height
    })
    let muglePoolLatestBlock = 0
    muglePoolRecentBlocks.forEach(block => {
      if (block.height > muglePoolLatestBlock && block.height < maxPoolBlockHeight) muglePoolLatestBlock = block.height
    })
    const minerBlockRewardData = getMinerBlockRewardData(muglePoolLatestBlock, networkData, muglePoolData, rigShareData, selectedRigWorker)
    const latestBlockMugleEarned = minerBlockRewardData ? minerBlockRewardData.userReward : 'n/a'
    ...
    let latestBlockMugleEarnedSyntax
    if (typeof latestBlockMugleEarned === 'number' && latestBlockMugleEarned > 0) {
      latestBlockMugleEarnedSyntax = `${latestBlockMugleEarned.toFixed(4)} MUGLE`
    } else if (latestBlock.height - Math.max(...mergedPoolBlocksMined) < 1) {
      latestBlockMugleEarnedSyntax = 'Calculating...'
    } else {
      latestBlockMugleEarnedSyntax = 'n/a'
    }
    const readableAmount = amount > 0 ? nanoMugleToMugle(amount).toFixed(4) : 0
    const minerImmatureBalanceSyntax = (!isNaN(minerImmatureBalance) && minerImmatureBalance > 0) ? `${nanoMugleToMugle(minerImmatureBalance).toFixed(4)} MUGLE` : 'n/a'
    ...
    const nextBlockMinerRewardData = getMinerBlockRewardData(maxPoolBlockHeight, networkData, muglePoolData, rigShareData, selectedRigWorker)

    const estimatedShareOfNextBlockReward = nextBlockMinerRewardData ? nextBlockMinerRewardData.userReward : 'n/a'
    const nextBlockRewardSyntax = typeof estimatedShareOfNextBlockReward === 'number' ? `${estimatedShareOfNextBlockReward.toFixed(4)} MUGLE` : 'n/a'
    ...
    const estimatedDailyEarningsSyntax = isNaN(estimatedDailyEarningsFromGps) ? 'n/a' : `${estimatedDailyEarningsFromGps.toFixed(4)} MUGLE`
            ...
            <Col xs={12} md={6} lg={6} xl={6}>
              <Table size='sm'>
                <tbody>
                  ...
                  <tr>
                    <td>Miner {BLOCK_RANGE}-Block Average Equivalence <span data-tip='The estimated gps you would need of each algorithm to earn an equivalent amount of MUGLE based on your average gps' style={{ verticalAlign: 'super', fontSize: '0.8rem' }}>?</span><ReactTooltip className={'customTheme'} /></td>
              ...
              <Table size='sm'>
                <tbody>
                  <tr className={'betaEstimates'}>
                    ...
                    <td>{latestBlockMugleEarnedSyntax}</td>
                  </tr>
                  <tr className={'betaEstimates'}>
                  ...
                  <tr style={{ display: isTotalRigWorker ? 'table-row' : 'none' }}>
                    ...
                    <td id={matureBalanceStyleId}>{readableAmount} MUGLE</td>
                  </tr>
                  <tr style={{ display: isTotalRigWorker ? 'table-row' : 'none' }}>
                  ...
                  <tr style={{ display: isTotalRigWorker ? 'table-row' : 'none' }}>
                    <td>Total Payouts</td>
                    <td>{nanoMugleToMugle(totalPayoutsAmount).toFixed(4)} MUGLE</td>
                  </tr>
                </tbody>
              </Table>
            </Col>
