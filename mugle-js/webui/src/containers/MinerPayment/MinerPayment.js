...
export class MinerPaymentComponent extends Component {
  ...
  onPaymentTypeChange = (event) => {
    ...
    if (paymentType === 'hotbit') {
      paymentType = 'manual'
      paymentMethod = 'http'
      recipient = 'https://mugle.hotbit.io:443/'
      ...
      paymentType = 'manual'
      paymentMethod = 'http'
      recipient = 'https://depositmugle.kucoin.com:443/deposit/'
    }
    this.setState({
      paymentType,
      paymentMethod,
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <FormGroup>
          ...
          <div style={{ textAlign: 'center' }}>
            <Alert color='warning' style={{ width: '80%', textAlign: 'center', display: 'inline-block' }}>Please be aware that automated payouts require a <strong>5 MUGLE</strong> minimum balance in order to be triggered.</Alert>
          </div>
        </FormGroup>
        <FormGroup check>
          <Label check>
        ...
        <Row>
          <Col xs={12} md={12} lg={6} xl={6}>
            <Card>
              <CardBody>
                <h4>Payout</h4>
                <p>MuglePool supports multiple methods of payment, including automatic payments and manual / on-demand payments. The list of payment methods is likely to grow, so stay tuned!</p>
                <br />
                <Alert color='danger' style={{ textAlign: 'center', position: 'relative', marginTop: '16px' }}>Please be aware that current MUGLE payment methods are still under development. We ask for your patience as we test and implement additional payment options.</Alert>
                <br />
                <h4>Examples</h4>
                <ul>
                  ...
                  <li><strong>HotBit</strong>:  <span style={{ color: C31_COLOR }}>https://mugle.hotbit.io<strong>:443</strong>/555555</span> <span style={{ display: 'block', float: 'right' }}>(Manual Payout -> Online Wallet / Port)</span></li>
