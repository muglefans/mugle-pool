...
import type { NetworkBlockData, MuglePoolBlockData, MinerShareData } from '../../types'
...
export type MinerBlockRewardsStateProps = {
  ...
  muglePoolData: Array<MuglePoolBlockData>
}
...
export class MinerBlockRewards extends Component<MinerBlockRewardsProps, MinerBlockRewardsState> {
  ...
  fetchData = () => {
    const { fetchBlockRange, fetchNetworkData, fetchMuglePoolData, fetchMinerShareData } = this.props
    const { range } = this.state
    fetchBlockRange(null, range)
    fetchNetworkData()
    fetchMuglePoolData()
    ...
    if (recentBlocks.length) {
      for (let i = 1; i <= range && i <= recentBlocks.length - 1; i++) {
        ...
        if (block.height > latestBlockHeight - 1440 + 250 && block.height < latestBlockHeight - 5) {
          const blockRewardData = getMinerBlockRewardData(block.height, networkData, muglePoolData, minerShareData)
          if (blockRewardData) {
        ...
        rows.push(
          <tr key={block.height}>
            <td style={{ textAlign: 'left' }}><a style={{ color: rowColor || C29_COLOR }} href={`https://mugleexplorer.net/block/${block.height}`} rel='noopener noreferrer' target='_blank' >{block.height}</a></td>
