...
import { nanoMugleToMugle, calculateDailyEarningFromGps } from '../../utils/utils.js'
...
export class MinerDataComponent extends Component<MinerDataComponentProps, MinerDataComponentState> {
  ...
  render () {
    const {
      ...
      nextBlockMugleEarned,
      latestBlockMugleEarned
    } = this.props
    const { faderStyleId, matureBalanceStyleId, immatureBalanceStyleId } = this.state
    ...
    if (typeof latestBlockMugleEarned === 'number' && latestBlockMugleEarned > 0) {
      latestBlockMugleEarnedSyntax = `${(nanoMugleToMugle(latestBlockMugleEarned) * 0.98).toFixed(4)} MUGLE`
    ...
    const minerImmatureBalanceSyntax = (!isNaN(minerImmatureBalance) && minerImmatureBalance > 0) ? `${nanoMugleToMugle(minerImmatureBalance * 0.98).toFixed(4)} MUGLE` : 'n/a'
    ...
    const nextBlockRewardSyntax = typeof nextBlockMugleEarned === 'number' ? `${(nanoMugleToMugle(nextBlockMugleEarned) * 0.98).toFixed(4)} MUGLE` : 'n/a'
    const estimatedDailyEarningsFromGps = calculateDailyEarningFromGps(networkData, minerData, latestBlock.height)
    const estimatedDailyEarningsSyntax = isNaN(estimatedDailyEarningsFromGps) ? 'n/a' : `${(estimatedDailyEarningsFromGps * 0.98).toFixed(4)} MUGLE`
    return (
