...
# Deploy Mugle-Pool to Google Cloud via Kubernetes

## Log into gcloud 
```
gcloud auth login
gcloud config set project muglepool-218920 
...
```
gcloud container clusters create muglepool  --enable-cloud-logging --disk-size=25G --machine-type=n1-standard-2 --num-nodes=6 --zone us-west1-c  
```
...
./rmq_secret.sh
kubectl create -f mugle_set.yaml   
...
./muglepool_secrets.sh
kubectl create -f dbInit.yaml
kubectl create -f blockWatcher.yaml
kubectl create -f mugleStats.yaml

./muglewallet_secret.sh
kubectl create -f muglewallet_claim.yaml 
kubectl create -f muglewallet.yaml
